...
def generate_pdf(summary: dict, narrative: str, output_path: str) -> None:
    ...
    # ── 2b. Financial Trend Charts ────────────────────────────────────────────
    # Charts are generated into temp PNG files and embedded via ReportLab Image.
    # If matplotlib is unavailable or generation fails the section is skipped
    # gracefully; chart_paths is collected here so files can be deleted after
    # doc.build() regardless of how build exits.
    chart_paths: list[str] = []
    try:
        from ace_research.charts import generate_charts, _MATPLOTLIB_AVAILABLE
        if _MATPLOTLIB_AVAILABLE:
            chart_paths = generate_charts(summary, years)
            _CW = 3.1 * inch
            _CH = 2.3 * inch
            story.append(Paragraph("Financial Trends", section_style))
            story.append(Spacer(1, 0.05 * inch))
            # Arrange 4 charts in a 2-column grid
            chart_rows = []
            for i in range(0, len(chart_paths), 2):
                ...
                for j in range(2):
                    ...
                    if idx < len(chart_paths):
                        row.append(Image(chart_paths[idx], width=_CW, height=_CH))
                    else:
                        row.append("")
                chart_rows.append(row)
            chart_table = Table(
                chart_rows,
                colWidths=[_CW + 0.15 * inch, _CW + 0.15 * inch],
            )
    ...
    # ── 5. Financial Efficiency & Returns ─────────────────────────────────────
    story.append(Paragraph("Financial Efficiency & Returns", section_style))
    ...
    try:
        ...
        # Delete temp chart PNG files regardless of build success/failure
        for _p in chart_paths:
            try:
                os.unlink(_p)
            except Exception:
                pass
